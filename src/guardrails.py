...
def _security_llm(provider: str | None = None, model: str | None = None):
    from .llm import get_chat_llm
    return get_chat_llm(provider=provider, model=model)


def llm_injection_check(
...
Return the structured assessment only.""",
    ...
    classifier = _security_llm(provider=provider, model=model).with_structured_output(InjectionAssessment)
...
def check_prompt_injection(
    ...
    if not settings.enable_llm_injection_check:
        return deterministic
    try:
        return llm_injection_check(text, provider=provider, model=model)
    except Exception:
...
def check_groundedness(
    answer: str, context: str, provider: str | None = None, model: str | None = None
...
Return the structured assessment only.""",
    ...
    judge = _security_llm(provider=provider, model=model).with_structured_output(GroundednessAssessment)
