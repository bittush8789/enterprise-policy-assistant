...
def _llm(provider: str | None = None, model: str | None = None) -> BaseChatModel:
...
class EnterprisePolicyAssistant:
    ...
    def ask(self, question: str) -> AskResult:
        ...
        injection = check_prompt_injection(question, provider=self.provider, model=self.model)
        ...
        response = _llm(provider=self.provider, model=self.model).invoke(
        ...
        if settings.enable_groundedness_check:
            try:
                assessment = check_groundedness(
                    raw_answer, context, provider=self.provider, model=self.model
                )
