...
def _quality_judge(
    ...
    llm = get_chat_llm(provider=provider, model=model).with_structured_output(AnswerQualityAssessment)
    return llm.invoke(
...
def run_evaluation(
    ...
    assistant = EnterprisePolicyAssistant(provider=provider, model=model)
